...
from smallfile_schema import (
    WorkloadParams,
    WorkloadResults,
    WorkloadError,
    smallfile_schema,
    output_params_schema,
    output_results_schema,
    output_rsptimes_schema,
)
...
def smallfile_run(
    ...
    try:
        with open(
            ...
            for row in rsptimes_csv:
                if (
                    not re.match("per-", row["host:thread"])
                    and not re.match("cluster-", row["host:thread"])
                    and not re.match("time-", row["host:thread"])
                ):
    ...
    return "success", WorkloadResults(
        output_params_schema.unserialize(smallfile_json["params"]),
        output_results_schema.unserialize(smallfile_json["results"]),
        output_rsptimes_schema.unserialize(smallfile_rsptimes),
    )
